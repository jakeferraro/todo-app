...
export const createTask = async (req, res) => {
  ...
  try {
    const insertedRows = await db('tasks').insert({ name, description, folder_id }).returning('*');
...
export const updateTask = async (req, res) => {
  ...
  const { name, description, folder_id } = req.body;
  try {
    const [updatedTask] = await db('tasks').where('id', id).update({ name, description, folder_id }).returning('*');
